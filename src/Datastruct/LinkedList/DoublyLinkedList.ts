...
export default class LinkedList<NodeData = any> {
	 ...
	 */
	public insertBefore(referenceNode: LinkedListNode<NodeData>, data: NodeData): LinkedList<NodeData> {
	...
	public sort(compare: (a: NodeData, b: NodeData) => boolean): LinkedList<NodeData> {
		...
		const quicksort = (start: LinkedListNode<NodeData>, end: LinkedListNode<NodeData>) => {
			if (start === end) {
				return;
			}
	 ...
	 */
	public insertAfter(referenceNode: LinkedListNode<NodeData>, data: NodeData): LinkedList<NodeData> {
	 ...
	 */
	public reduce(
		f: (accumulator: any, currentNode: NodeData, index: number, list: LinkedList<NodeData>) => any,
		start?: any,
		reverse = false
	): any {
		let currentIndex = reverse ? this.length - 1 : 0;
