...
const breakCheck = [
  () =>
    current.flags[DamageType.Pure] ||
    skipEngine ||
    current.flags[DamageType.INTERNAL],
  () =>
    current.damage <= 0 ||
    current.flags[DamageType.Pure] ||
    current.flags[DamageType.INTERNAL],
...
function addRecursive(d: DamageInstance) {
  ...
  if (
    !kicking &&
    recursiveSource.get(d.source) &&
    recursiveTarget.get(d.target)
  ) {
...
function doPreEvent(d: DamageInstance, isNatural: boolean) {
  ...
  skipEngine =
    d.damageType == DAMAGE_TYPE_UNKNOWN || d.flags[DamageType.INTERNAL];
...
function runRecursive(i: number, stack: DamageInstance[]) {
  ...
  for (const j of $range(i, ex)){
    ...
    if (d.damage > 0.0) {
      ...
      if (d.damage < 0)
        SetWidgetLife(d.target, GetWidgetLife(d.target) - d.damage);
    }
    afterDamage();
  }
  return ex;
...
export class Damage {
  ...
  public static register(
    whichEvent: DamageEvent,
    priority: number,
    callback: DamageAction
  ) {
